...
def sign():
    ...
    Entry(Sign_up_window,width=50,show="*",textvariable=password).place(x=170,y=400)
    #####################  Radiobutton ##################################
    ...
    def Create_account():
        ...
        if email[:at_the_rate] == '':
            messagebox.showwarning('','Invalid email')
            return
        conn = sq.connect('Server.db')
        cur = conn.cursor()
        try:
            cur.execute(f'insert into data(name,email,password,salary,residence,age,occupation,gender,subscribe) values("{Name.get()}","{Email.get()}","{password.get()}","{salary.get()}","{residence.get()}",{age.get()},"{occupation.get()}","{gender.get()}",{sub.get()})')
        except:
        ...
        Entry(Sign_up_window,width=50,show="*",textvariable=password).delete(0,END)
        gender_box.current(0)
        salary_box.current(0)
    ##################################  Button ######################################
...
def Sign_in():
    ...
    Entry(Sign_in_window,width=50,show="*",textvariable=password).place(x=170,y=150)
    ###################  Function ###############################
    def log_in():
        if Email.get()=='':
            messagebox.showwarning('','Fill the email')
        ...
        for i in cur.fetchall():
            if (email==i[0]) and (password1==i[1]):
                messagebox.showinfo('',f'Welcome {i[2]}')
                conn.commit()
                conn.close()
                return
            if (email==i[0]) and (password1!=i[1]):
